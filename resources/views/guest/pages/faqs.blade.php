...
@section('content')
    <main id="main-content">
        <div class="transition-opacity duration-1000 opacity-100 font-sans text-educave-975 bg-educave-50 selection:bg-educave-200">
                                ...
                                <div class="space-y-3" role="tablist" aria-label="FAQ topics">
                                    @foreach($faqTopics as $key => $topic)
                                        <button type="button" role="tab" id="faq-tab-{{ $key }}"
                                                class="faq-topic-btn w-full group text-left p-6 rounded-[24px] border transition-all duration-500 flex items-center justify-between {{ $loop->first ? 'bg-educave-900 border-educave-900 text-white shadow-2xl shadow-educave-900/20' : 'bg-white border-educave-900/5 text-educave-900 hover:border-educave-900/20' }}"
                                                data-faq-topic="{{ $key }}" data-faq-label="{{ $topic['label'] }}"
                                                aria-selected="{{ $loop->first ? 'true' : 'false' }}">
                                            <div class="flex items-center gap-4">
                                                <div class="faq-topic-dot w-2 h-2 rounded-full transition-all duration-500 {{ $loop->first ? 'bg-pjp-yellow shadow-[0_0_0_2px_rgba(242,230,49,0.35)]' : 'bg-educave-900/10 group-hover:bg-educave-900/25' }}"></div>
                                    ...
                                    <div class="space-y-4">
                                        @foreach($topic['items'] as $item)
                                            @php
                                                ...
                                                $isOpen = $loop->first;
                                            @endphp
                                            <div @class(['faq-accordion-item overflow-hidden rounded-[32px] border transition-all duration-700', 'bg-white border-educave-900/15 shadow-xl' => $isOpen, 'bg-transparent border-educave-900/5 hover:border-educave-900/15' => !$isOpen])>
                                                <button type="button" class="faq-accordion-trigger w-full text-left p-8 md:p-10 flex items-start justify-between gap-6 group" aria-expanded="{{ $isOpen ? 'true' : 'false' }}">
                                                    <div class="flex gap-6">
                                                        <span @class(['faq-acc-num text-xl font-serif font-bold transition-all duration-500', 'text-educave-900 scale-125' => $isOpen, 'text-educave-900/20' => !$isOpen])>{{ $idx }}</span>
                                                        <span @class(['faq-acc-title text-xl md:text-2xl font-serif font-bold leading-tight transition-colors duration-500', 'text-educave-900' => $isOpen, 'text-educave-900/70 group-hover:text-educave-900' => !$isOpen])>{{ $item['q'] }}</span>
                                                    </div>
                                                    <div @class(['faq-accordion-icon shrink-0 w-12 h-12 rounded-full border border-educave-900/10 flex items-center justify-center transition-all duration-700', 'bg-educave-900 border-educave-900 text-white rotate-45' => $isOpen, 'bg-white text-educave-900 group-hover:bg-educave-50' => !$isOpen])>
            ...
            <section class="padding-custom bg-educave-100/50">
                <div class="container mx-auto px-6 md:px-12 lg:px-24">
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 text-center lg:text-left">
                        <div>
                            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-map-pin text-educave-900 mb-6 mx-auto lg:mx-0" aria-hidden="true"><path d="M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0"/><circle cx="12" cy="10" r="3"/></svg>
                        ...
                        <div>
                            ...
                            <div class="flex justify-center lg:justify-start gap-3 flex-wrap">
                                @foreach(['WAEC', 'NECO', 'JAMB CBT', 'Catholic'] as $badge)
                                    <div class="px-3 py-1 rounded-lg bg-educave-900/10 text-educave-900 text-[10px] font-black tracking-widest uppercase">{{ $badge }}</div>
                                @endforeach
                            </div>
                        </div>
                    </div>
...
@push('scripts')
    <script>
        (function () {
            ...
            const ACC_ITEM_OPEN    = 'faq-accordion-item overflow-hidden rounded-[32px] border transition-all duration-700 bg-white border-educave-900/15 shadow-xl';
            const ACC_ITEM_CLOSED  = 'faq-accordion-item overflow-hidden rounded-[32px] border transition-all duration-700 bg-transparent border-educave-900/5 hover:border-educave-900/15';
            const NUM_OPEN         = 'faq-acc-num text-xl font-serif font-bold transition-all duration-500 text-educave-900 scale-125';
            ...
            const TITLE_CLOSED     = 'faq-acc-title text-xl md:text-2xl font-serif font-bold leading-tight transition-colors duration-500 text-educave-900/70 group-hover:text-educave-900';
            const ICON_OPEN        = 'faq-accordion-icon shrink-0 w-12 h-12 rounded-full border border-educave-900/10 flex items-center justify-center transition-all duration-700 bg-educave-900 border-educave-900 text-white rotate-45';
            ...
            const TOPIC_ACTIVE     = 'faq-topic-btn w-full group text-left p-6 rounded-[24px] border transition-all duration-500 flex items-center justify-between bg-educave-900 border-educave-900 text-white shadow-2xl shadow-educave-900/20';
            const TOPIC_INACTIVE   = 'faq-topic-btn w-full group text-left p-6 rounded-[24px] border transition-all duration-500 flex items-center justify-between bg-white border-educave-900/5 text-educave-900 hover:border-educave-900/20';
            ...
            const DOT_INACTIVE     = 'faq-topic-dot w-2 h-2 rounded-full transition-all duration-500 bg-educave-900/10 group-hover:bg-educave-900/25';
