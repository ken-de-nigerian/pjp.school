@extends('layouts.app', ['title' => 'Behaviour'])

@section('content')
    <main class="flex-1 flex flex-col min-h-0 w-full overflow-y-auto overflow-x-hidden overscroll-y-none pb-24 lg:pb-8 scrollbar-hide" style="background: var(--surface);">
        <div class="page-content flex-1 flex flex-col w-full max-w-7xl mx-auto min-w-0 px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10">
            <x-admin.hero-page
                aria-label="Teacher behavioural analysis overview"
                pill="Teacher"
                title="Behavioural Analysis"
                description="Take or view behavioural analysis for the current term."
            >
                <x-slot name="actions">
                    <a href="{{ route('teacher.behavioral.view') }}" class="admin-dashboard-hero__btn admin-dashboard-hero__btn--primary w-full lg:w-auto justify-center min-h-[44px] sm:min-h-0">
                        <i class="fas fa-eye text-[10px] sm:text-xs" aria-hidden="true"></i>
                        <span>View Behavioural</span>
                    </a>
                </x-slot>
            </x-admin.hero-page>
                        ...
                        <div class="h-full min-h-[200px]">
                            <div class="flex flex-col h-full overflow-hidden rounded-2xl transition-all duration-200" style="background: var(--surface-container-lowest); border: 1px solid var(--outline-variant);">
                                <div class="p-5 sm:p-6 text-center flex-1 flex flex-col items-center justify-center gap-3">
                                    ...
                                    <p class="text-2xl sm:text-3xl font-normal tracking-tight mb-0" style="color: var(--on-surface);">{{ $c['user_count'] ?? 0 }}</p>
                                ...
                                <div class="p-4 sm:p-5 pt-0 flex justify-center" style="border-top: 1px solid var(--outline-variant);">
                                    @php
                                        ...
                                        $takeUrl = route('teacher.behavioral.take') . '?' .
                                            'class=' . $raw($c['class_name'] ?? '') . '&term=' . $raw($settings['term'] ?? 'First Term') .
                                            '&session=' . $raw($settings['session'] ?? '');
                                    @endphp
                    ...
                    @empty
                        <div class="col-span-full flex-1 flex flex-col items-center justify-center min-h-[min(400px,50vh)] py-12 sm:py-16">
                            <div class="rounded-3xl p-8 sm:p-12 text-center w-full max-w-lg" style="background: var(--surface-container-lowest); border: 1px solid var(--outline-variant);">
                                ...
                                <p class="text-sm font-normal mb-0" style="color: var(--on-surface-variant);">You currently have no classes assigned for behavioural analysis.</p>
                            </div>
                        </div>
                    @endforelse
                </div>
