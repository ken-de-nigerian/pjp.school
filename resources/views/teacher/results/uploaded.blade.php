        ...
        <div class="page-content flex-1 flex flex-col w-full max-w-7xl mx-auto min-w-0 px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10">
            @php
                $uploadedHeroDescription = $hasFilters
                    ? 'View and ' . ($canEdit ? 'edit' : 'review') . ' results for this class and subject.'
            ...
            >
                <x-slot name="above">
                    <a href="{{ route('teacher.results.index') }}" class="admin-page-hero__back mb-2 sm:mb-0">
                ...
                @if($hasFilters)
                    <x-slot name="actions">
                        <a href="{{ route('teacher.uploaded.index') }}" class="admin-dashboard-hero__btn w-full lg:w-auto justify-center min-h-[44px] sm:min-h-0">
                ...
                <div class="rounded-3xl p-4 sm:p-5 lg:p-6 mb-6 overflow-hidden min-w-0 w-full" style="background: var(--surface-container-low); border: 1px solid var(--outline-variant);">
                    <form method="GET" action="{{ route('teacher.uploaded.index') }}" class="space-y-4 sm:space-y-5">
                        ...
                        <div class="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 pt-2 min-w-0" style="border-top: 1px solid var(--outline-variant); padding-top: 1.25rem;">
                            <a href="{{ route('teacher.uploaded.index') }}" class="btn-secondary inline-flex items-center justify-center gap-2 px-6 py-3 min-h-[2.75rem] sm:min-h-0 min-w-[140px] rounded-xl text-sm font-medium transition-all duration-200 sm:min-w-[120px]" style="border-radius: 12px;">
                                ...
                                @foreach($results as $r)
                                    ...
                                    <li class="flex flex-col gap-0 rounded-2xl border p-4 md:rounded-none md:border-0 md:border-b md:border-t-0 md:flex-row md:items-center md:gap-4 md:py-4 md:px-5 lg:px-6 md:min-w-0 md:p-0 transition-[background-color] duration-200" style="background: var(--surface-container-lowest); border-color: var(--outline-variant);" data-result-id="{{ $r->id }}" data-student-id="{{ $r->studentId ?? '' }}">
                                        ...
                                        <div class="mt-2 pt-2 sm:mt-3 sm:pt-3 border-t md:border-t-0 md:mt-0 md:pt-0 w-full flex flex-row items-center justify-between gap-3 md:contents" style="border-color: var(--outline-variant);">
                                            ...
                                            @else
                                                <span class="inline-flex items-center gap-2 px-2 py-1 rounded-full text-xs" style="background: var(--surface-container-high); color: var(--on-surface-variant);">
            ...
            <script>
                (function () {
                    ...
                    form.addEventListener('submit', function (e) {
                        ...
                        fetch(@json(route('teacher.results.edit')), {
                            method: 'PUT',
