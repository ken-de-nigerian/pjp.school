...
<body>
    ...
    <h1>Entrance Examination Applicants — {{ now()->format('Y') }}</h1>
    <p class="meta">Generated {{ now()->format('j M Y, g:i A') }} · {{ $applicants->count() }} applicant(s)</p>
    ...
    <table>
        ...
        <tbody>
            @forelse($applicants as $index => $a)
                ...
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>{{ e($a->uniqueID ?? $a->id ?? '—') }}</td>
                    <td>{{ $name ?: '—' }}</td>
                    <td>{{ e($a->selectgender ?? '—') }}</td>
                    <td>{{ e($a->candidates_date_of_birth ?? '—') }}</td>
                    <td>{{ e($a->candidates_current_school ?? '—') }}</td>
                    <td>{{ e($a->candidates_current_class ?? '—') }}</td>
                    <td>{{ e(($a->states ?? '') . ' / ' . ($a->candidates_lga ?? '—')) }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="8" style="text-align: center; color: #666;">No applicants.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
