...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-semibold mt-2">Take behavioural — {{ $class }}</h1>
</div>
<form id="behavioral-form" class="bg-white rounded-lg shadow overflow-hidden">
    @csrf
    <input type="hidden" name="_target" value="{{ url('requests/take_teacher_behavioral') }}">
            ...
            <tbody>
                @foreach($students as $s)
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-2"><input type="text" name="students[{{ $loop->index }}][neatness]" value="" class="rounded border w-20" placeholder="1-5"></td>
                        <td class="px-4 py-2"><input type="text" name="students[{{ $loop->index }}][punctuality]" value="" class="rounded border w-20" placeholder="1-5"></td>
                        <td class="px-4 py-2"><input type="text" name="students[{{ $loop->index }}][politeness]" value="" class="rounded border w-20" placeholder="1-5"></td>
                        ...
                        <input type="hidden" name="students[{{ $loop->index }}][music]" value="">
                        <input type="hidden" name="students[{{ $loop->index }}][sports]" value="">
                        <input type="hidden" name="students[{{ $loop->index }}][attentiveness]" value="">
                        <input type="hidden" name="students[{{ $loop->index }}][health]" value="">
                    </tr>
                @endforeach
            </tbody>
        </table>
...
<p id="behavioral-msg" class="mt-2 text-sm hidden"></p>
<script>
document.getElementById('behavioral-form').addEventListener('submit', function(e) {
    e.preventDefault();
    const form = e.target;
    const fd = new FormData(form);
    const msg = document.getElementById('behavioral-msg');
