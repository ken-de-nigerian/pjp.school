            ...
            <div class="rounded-3xl p-4 sm:p-5 lg:p-6 mb-6 overflow-hidden min-w-0 w-full" style="background: var(--surface-container-low); box-shadow: var(--elevation-1); border: 1px solid var(--outline-variant);">
                @if($news->count() > 0)
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
                        <div>
                            <h2 class="text-lg sm:text-xl lg:text-2xl font-bold mb-1" style="color: var(--text-primary);">
                                All Announcements
                            </h2>
                            <p class="text-sm" style="color: var(--text-secondary);">
                                Latest school notices and updates.
                            </p>
                        </div>

                        <p class="text-xs sm:text-sm font-medium px-3 py-1.5 rounded-full" style="background: var(--surface-container-high); color: var(--on-surface-variant);">
                            {{ $news->total() }} total
                        </p>
                    </div>
                @endif
                ...
                @else
                    <div class="flex-1 flex flex-col min-h-0">
                        ...
                        @if($news->hasPages())
                            <div class="mt-6 pt-4 border-t" style="border-color: var(--outline-variant);">
                ...
                <div id="news-delete-modal" class="fixed inset-0 z-50 hidden overflow-y-auto overscroll-contain" aria-modal="true" role="dialog" aria-labelledby="news-delete-modal-title">
                    ...
                    <div class="relative min-h-full flex items-center justify-center p-4 py-6 sm:p-6">
                        <div class="relative w-full max-w-md min-w-0 max-h-[calc(100dvh-2rem)] overflow-y-auto overscroll-contain rounded-xl py-5 px-4 sm:py-6 sm:px-6 shadow-xl border my-auto" style="background: var(--surface-container-lowest); border-color: var(--outline-variant);">
                            ...
                            <p id="news-delete-modal-message" class="text-sm mb-6" style="color: var(--on-surface-variant);">Are you sure you want to delete this announcement? This action cannot be undone.</p>
        ...
        @push('scripts')
            <script>
                (function() {
                    ...
                    const deleteModalTitle = document.getElementById('news-delete-modal-title');
                    const deleteModalMessage = document.getElementById('news-delete-modal-message');
