    ...
    <main class="flex-1 overflow-y-auto overflow-x-hidden overscroll-y-none pb-24 lg:pb-8 scrollbar-hide" style="background: var(--surface);">
        <div class="max-w-7xl mx-auto min-w-0 px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
            ...
            <x-admin.hero-page
                ...
                pill="Admin"
                :title="$fullName ?: '—'"
                :description="e($student->reg_number) . ' · ' . e($student->class ?? '—')"
            >
                <x-slot name="above">
                ...
                <x-slot name="below">
                    <div class="flex items-center gap-4 pt-1">
                        <img class="w-16 h-16 sm:w-20 sm:h-20 rounded-full object-cover border-2 flex-shrink-0" style="border-color: var(--outline-variant);" src="{{ $profileImageSrc }}" alt="" onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($fullName ?: 'S') }}&size=160'">
                    </div>
                </x-slot>
                <x-slot name="actions">
                    <a href="{{ route('admin.students.edit', $student->id) }}" class="admin-dashboard-hero__btn admin-dashboard-hero__btn--primary w-full lg:w-auto justify-center min-h-[44px] sm:min-h-0">
