@extends('layouts.app', ['title' => 'Promote and demote'])
...
@section('content')
            ...
            <div class="flex-1 flex flex-col min-h-0 w-full rounded-3xl p-5 sm:p-6 lg:p-8" style="background: var(--surface-container-low); box-shadow: var(--elevation-1);">
                <div class="col-span-full flex-1 flex flex-col items-center justify-center py-12 sm:py-16">
                    <div class="rounded-3xl p-4 sm:p-6 lg:p-8 overflow-hidden min-w-0 w-full" style="background: var(--surface-container-low); box-shadow: var(--elevation-1); border: 1px solid var(--outline-variant);">
                        <form action="{{ route('admin.students.demote') }}" method="POST" class="p-5 sm:p-6 space-y-5" id="demote-form">
                            @csrf

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
                                <div class="md:col-span-1 form-group">
                                    <label for="demoteStudentFrom" class="form-label">Demote from this class</label>
                                    <x-forms.md-select-native id="demoteStudentFrom" name="from_class" class="form-select" data-load-students>
                                ...
                                <div class="md:col-span-1 form-group">
                                    <label for="demoteStudentTo" class="form-label">To this class</label>
                                    <x-forms.md-select-native id="demoteStudentTo" name="to_class" class="form-select">
                                        <option value="">Select a class</option>
                                        @foreach(array_reverse($getClasses) as $class)
                                            <option value="{{ e($class->class_name) }}">{{ e($class->class_name) }}</option>
                                        @endforeach
                        ...
                        <div class="form-group">
                            ...
                            <div id="studentId" class="rounded-2xl border overflow-hidden max-h-64" style="border-color: var(--outline-variant); background: var(--surface-container-lowest);">
                                <div class="px-4 py-2 border-b text-xs sm:text-sm flex items-center justify-between gap-2"
                                    ...
                                    <div class="flex items-center gap-2">
                                        <button type="button" id="demote-select-all" class="text-[11px] sm:text-xs font-medium px-2 py-1 rounded-lg transition-colors" style="color: var(--primary); background: var(--primary-container);">
                                            Select all
                                        </button>
                                        <button type="button" id="demote-clear" class="text-[11px] sm:text-xs font-medium px-2 py-1 rounded-lg transition-colors" style="color: var(--on-surface-variant); background: var(--surface-container-high);">
                                            Clear
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div class="pt-2 border-t" style="border-color: var(--outline-variant);">
                                <button type="submit" id="demoteBtn" class="btn-primary w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-sm font-medium transition-all duration-200 hover:opacity-95 active:scale-[0.98]" style="border-radius: 999px;">
                                    <i class="fas fa-arrow-down-long text-xs" aria-hidden="true"></i>
    ...
    @push('scripts')
        <script>
            ...
            const fromSelect = document.getElementById('demoteStudentFrom');
            ...
            const demoteFieldIds = ['from_class', 'to_class', 'student_ids'];
            if (form && demoteBtn) {
                ...
                form.addEventListener('submit', function (e) {
                    e.preventDefault();
                    if (typeof clearFieldErrors === 'function') clearFieldErrors(demoteFieldIds);
                    ...
                    if (typeof setButtonLoading === 'function') setButtonLoading(demoteBtn, true);
                    ...
                    fetch(form.action, {
                        ...
                        if (data.status === 'success' && typeof flashSuccess === 'function') {
                            flashSuccess(data.message || 'Students demoted.');
                            setTimeout(function () {
                                window.location.reload();
                            }, window.RELOAD_DELAY_MS);
                        } else if (data.status === 'error' && typeof flashError === 'function') {
                            flashError(data.message || 'Unable to demote students.');
                        }
                    }).catch(function (err) {
                        if (err.message !== 'Validation failed' && typeof flashError === 'function') {
                            flashError('An error occurred while demoting students.');
                        }
                    }).finally(function () {
                        if (typeof setButtonLoading === 'function') setButtonLoading(demoteBtn, false);
                    });
                });
            }
        });
