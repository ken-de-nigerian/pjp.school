...
@section('content')
    <main class="flex-1 flex flex-col min-h-0 w-full overflow-y-auto overflow-x-hidden overscroll-y-none pb-24 lg:pb-8 scrollbar-hide" style="background: var(--surface);">
        <div class="page-content flex-1 flex flex-col w-full max-w-7xl mx-auto min-w-0 px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10">
            ...
            <x-admin.hero-page
                aria-label="Take attendance overview"
                pill="Admin"
                title="Take Attendance"
                description="Select a class to mark attendance for the current term."
            >
                @if(Route::has('admin.attendance.view') && $canViewAttendance)
                    <x-slot name="actions">
                        <a href="{{ route('admin.attendance.view') }}" class="admin-dashboard-hero__btn admin-dashboard-hero__btn--primary w-full lg:w-auto justify-center min-h-[44px] sm:min-h-0">
                            ...
                            <div class="flex flex-col h-full overflow-hidden rounded-2xl transition-all duration-200" style="background: var(--surface-container-lowest); border: 1px solid var(--outline-variant);">
                                ...
                                <div class="p-4 sm:p-5 pt-0 flex justify-center" style="border-top: 1px solid var(--outline-variant);">
                                    @php
                                        ...
                                        $takeUrl = route('admin.attendance.take') . '?' . 'class=' . $raw($c['class_name']) . '&term=' . $raw($settings['term']) . '&session=' . $raw($settings['session']);
                                    @endphp
                    ...
                    @empty
                        <div class="col-span-full flex-1 flex flex-col items-center justify-center min-h-[min(400px,50vh)] py-12 sm:py-16">
                            <div class="rounded-3xl p-8 sm:p-12 text-center w-full max-w-lg" style="background: var(--surface-container-lowest); border: 1px solid var(--outline-variant); box-shadow: var(--elevation-1);">
                                ...
                                <h2 class="text-xl font-normal tracking-tight mb-2" style="color: var(--on-surface);">No classes found</h2>
                                <p class="text-sm font-normal mb-8" style="color: var(--on-surface-variant);">Add classes and assign students to take attendance for the current term.</p>
                                @if(Route::has('admin.students.houses'))
                                    <div class="flex justify-center">
                                        <a href="{{ route('admin.students.houses') }}"
                                           class="btn-primary inline-flex items-center justify-center gap-2 px-8 py-3 min-w-[180px] rounded-xl font-medium text-sm transition-all duration-200 hover:opacity-95 active:scale-[0.98]" style="border-radius: 12px;">
                                            <i class="fas fa-chalkboard text-sm" aria-hidden="true"></i>
                                            Manage classes
                                        </a>
                                    </div>
                                @endif
