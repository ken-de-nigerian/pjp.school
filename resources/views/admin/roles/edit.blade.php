    ...
    <main class="flex-1 flex flex-col min-h-0 w-full overflow-y-auto overflow-x-hidden overscroll-y-none pb-24 lg:pb-8 scrollbar-hide" style="background: var(--surface);">
        <div class="page-content flex-1 flex flex-col w-full max-w-6xl mx-auto min-w-0 px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10">
            <div class="mb-4 sm:mb-6 w-fit">
                <a href="{{ route('admin.roles.index') }}" class="inline-flex items-center gap-2 text-sm font-medium transition-opacity hover:opacity-80" style="color: var(--on-surface-variant);">
            ...
            <header class="mb-5 lg:mb-6">
                <h1 id="role-page-title" class="text-2xl sm:text-3xl font-normal tracking-tight mb-1" style="color: var(--on-surface); letter-spacing: -0.02em;">{{ e($role->name) }}</h1>
            ...
            <form action="{{ route('admin.roles.update', $role) }}" method="POST" id="role-form" class="min-w-0 flex flex-col gap-4 sm:gap-5" data-role-form="edit" data-total-perms="{{ $totalPerms }}">
                ...
                <div class="role-m3-card overflow-hidden">
                    <div class="role-m3-card-header px-4 sm:px-5 py-3.5 flex items-center justify-between gap-3 flex-wrap">
                        <div class="flex items-center gap-3 min-w-0">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-xs mt-0.5" style="color: var(--on-surface-variant);">How this role appears when assigning staff</p>
                            </div>
                        </div>
                        <div id="role-enabled-badge" class="flex items-baseline gap-1.5 tabular-nums shrink-0 py-1 px-3 rounded-lg" style="background: var(--surface-container-lowest); border: 1px solid var(--divider);">
                            <span id="role-enabled-count" class="text-lg font-medium" style="color: var(--on-surface);">{{ $enabledCount }}</span>
                    ...
                    <div class="px-4 sm:px-5 py-4" style="background: var(--surface-container-lowest);">
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $role->name) }}" placeholder="e.g. Senior teacher" class="form-input w-full min-w-0 rounded-xl py-2.5 px-3 text-sm" autocomplete="off" style="background: var(--surface-container-low); border: 1px solid var(--divider);" aria-describedby="name-error">
                    ...
                    fetch(form.action, {
                        ...
                        .then(function (r) {
                            if (r.ok && r.data.status === 'success') {
                                ...
                                if (r.data.role) {
                                    if (pageTitle) pageTitle.textContent = r.data.role.name;
                                    if (enabledCountEl) enabledCountEl.textContent = String(r.data.role.enabled_count);
                                }
                                return;
                            }
                            if (r.status === 422 && r.data.errors) {
                                ...
                                const msg = keys.length ? r.data.errors[keys[0]][0] : 'Check the form.';
